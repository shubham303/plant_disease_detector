...
class PlantScanHistoryScreen extends StatefulWidget {
  final Plant plant;

  const PlantScanHistoryScreen({super.key, required this.plant});

  @override
  State<PlantScanHistoryScreen> createState() => _PlantScanHistoryScreenState();
}

class _PlantScanHistoryScreenState extends State<PlantScanHistoryScreen> {
  ...
  Future<void> _loadScanHistory() async {
    ...
    try {
      final history = await ScanHistoryService.getScanHistoryForPlant(widget.plant.id);
      
      // Generate dummy data if no history exists
      if (history.isEmpty) {
        await ScanHistoryService.generateDummyScanHistory(widget.plant.id);
        final updatedHistory = await ScanHistoryService.getScanHistoryForPlant(widget.plant.id);
        setState(() {
          _scanHistory = updatedHistory;
          _isLoading = false;
        });
      } else {
        setState(() {
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              'Scan your plant to detect diseases',
                              style: GoogleFonts.inter(
                                fontSize: 14,
                                color: Colors.grey[600],
                              ),
